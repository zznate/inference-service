...
use crate::config::{LoggingConfig, LogOutput, LogFormat, RotationPolicy};
    ...
    match config.output {
        ...
        LogOutput::File => {
            if let Some(file_config) = &config.file {
                ...
                match config.format {
                    LogFormat::Pretty => subscriber.with(fmt::layer().pretty().with_writer(writer)).init(),
                    LogFormat::Json => subscriber.with(fmt::layer().json().with_writer(writer)).init(),
                    LogFormat::Compact => subscriber.with(fmt::layer().compact().with_writer(writer)).init(),
                }
            } else {
...
fn create_file_writer(file_config: &crate::config::FileLoggingConfig) -> (NonBlocking, WorkerGuard) {
    // Create directory
    if let Err(e) = fs::create_dir_all(&file_config.directory) {
        eprintln!("Failed to create log directory: {}. Using stdout.", e);
        return tracing_appender::non_blocking(std::io::stdout());
    }
    
    // Create rolling file appender
    let appender = match file_config.rotation_policy {
        RotationPolicy::Daily => tracing_appender::rolling::daily(
            &file_config.directory,
            &file_config.prefix
        ),
        RotationPolicy::Hourly => tracing_appender::rolling::hourly(
            &file_config.directory,
            &file_config.prefix
        ),
        RotationPolicy::Size => {
